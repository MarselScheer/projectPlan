...
Note the yellow "U" for every tasks. This indicates that the current task
has the label unscheduled. The reason is that we not explicitly defined
a start date. Hence, the plan was calculated assuming that it starts today.
See vignett for "Start and End" for more details.
...
Note that "get paid and pick up of the kitchen" was forced by us to be in status unscheduled,
by placing the tag "unscheduled" in the status-column in the spreadsheet. This automatically
puts "note to Life&Cook (1 week before installation) ..." also into unscheduled. 
See vignette "Start and End" for more details.